...
uv run image "英文生图提示词" --n 4 --image-file "模特图" --image-file "产品图1" --image-file "产品图2" --output-dir "output/<本次运行时间>/"
```

## Key Rule

- 用户说的数量记为“请求张数”；未指定时默认请求张数为 9。
- 每 1 个请求张数生成 1 条英文生图提示词，并调用 1 次 `uv run image --n 4`。
- 实际输出图片数 = 请求张数 × 4。例如用户说 6 张，就生成 6 条提示词、调用 6 次 `uv run image --n 4`，最终输出 24 张。
- `uv run chat` 生成的 JSON 数组长度等于请求张数，不等于乘以 4 后的输出图片数。
